...
def generate_pdf_itinerary(rows, grand_total):
    ...
    pdf.cell(200, 10, text="Family Road Trip Itinerary", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.set_font("helvetica", "I", 10)
    pdf.cell(200, 10, text=f"Estimated Total Budget: £{grand_total:,.2f}", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.ln(10)
    
    for row in rows:
        pdf.set_font("helvetica", "B", 12)
        pdf.cell(200, 8, text=f"Leg {row['Leg']}: {row['Route Stretch']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        
        pdf.set_font("helvetica", "", 10)
        pdf.cell(200, 6, text=f"Start Date: {row['Start Date']} | Check-in: {row['Check-in Date'][:10]}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.cell(200, 6, text=f"Distance: {row['Distance']} | Driving Time: {row['Driving Time']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.cell(200, 6, text=f"Pace: {row['Pace & Status']}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(5)
...
# ==============================================================================
total_legs_count = max(1, len(route_list) - 1)
active_leg = min(st.session_state.selected_leg_idx, max(0, len(route_list) - 2))
st.session_state.selected_leg_idx = active_leg
progress_percent = active_leg / total_legs_count if len(route_list) >= 2 else 0.0
...
with col2:
    ...
    if st.button("Calculate Trip Timeline", type="primary", width='stretch', disabled=len(route_list) < 2):
    ...
    if st.session_state.run_analysis:
        ...
        origin_row = build_origin_itinerary_summary(route_list[0].name, start_date, REST_DAYS.get(route_list[0].name, 0))
        itinerary_rows.append(origin_row)
        ...
        with sniper_placeholder.container():
            ...
            if itinerary_rows:
                ...
                for idx, row in enumerate(itinerary_rows[1:], start=1):
                    dest = route_list[idx]
    ...
    # ==============================================================================
    if st.session_state.get("run_analysis", False) and itinerary_rows and len(route_list) >= 2:
        leg_idx = st.session_state.selected_leg_idx
        orig, dest = route_list[leg_idx], route_list[leg_idx + 1]
        active_schedule = itinerary_rows[leg_idx + 1]

        if leg_idx == 0:
            origin_schedule = itinerary_rows[0]
            st.markdown(f"## 🏡 Origin / Arrival Details: {orig.name}")
            st.caption(f"Arrival date: {origin_schedule['Check-in Date']}")

            with st.container(border=True):
                st.markdown(f"### City Guide: {orig.name}")
                st.markdown(get_cached_location_insights(orig.name, scenic_mode))

            origin_col1, origin_col2 = st.columns(2)
            with origin_col1:
                with st.expander("☀️ Atlanta Daylight Hours", expanded=False):
                    daylight_hours = calculate_approx_daylight(orig.latitude, origin_schedule["Day of Year"])
                    st.markdown(
                        f"Atlanta will have approximately **{daylight_hours} hours** of daylight on "
                        f"{origin_schedule['Start Date']}."
                    )
            with origin_col2:
                with st.expander("⚠️ Atlanta Safety & Awareness", expanded=False):
                    st.markdown(get_cached_safety_alerts(orig.name))
